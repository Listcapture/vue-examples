...
<script setup lang="ts">
  ...
  import { AsignedCourse, AsignedCourseList, Course, CourseLIST, CourseList, Lab, lablist } from './projectEntity';
  import $ from 'jquery';

  const showModal = ref(false);
  let updateModal=ref(false);
  const isDisabled=ref(true);
  let updateModalCourseId=ref(0);
  ...
  let tAsignedCourseParameter=ref({StartWeek:0,EndWeek:0,teachingDay:0,StartTime:0,EndTime:0});

  let updateModalCourse=reactive(courselist.getAllCourses()[selectedCourseId.value]);
 // 定义数据
...
const closeUpdateModal=()=>{
  ...
  const Class: number[] = [1, 2, 3, 4, 5, 6, 7, 8];
  ...
  let curLab:Lab = new Lab(lablistProxy.getLabList.length + 1, '901', new AsignedCourseList([]));
  
   let ObjLab:AsignedCourse[]= reactive(lablist.getLabList()[(curLabIndex.value<0?0:curLabIndex.value)]).getAsignedCourseList().getAllAsignedCourse();
  ...
  let getWeekofWeekSelect= function(){
    curWeek.value=$('#Week')?.val();
    if($('#Week')?.val()) curWeek.value=$('#Week').val();
    console.log(curWeek.value);
  }
  let getCourseSelect = function() {
  selectedCourseId.value = $('#course').val();
  console.log("Selected Course ID: " + selectedCourseId.value);
  handleConfirmation();
};


 let  getBackgroundColor=(dayIndex, timeIndex):string => {
    let cweek=curWeek.value;
    let clab:AsignedCourse[]=lablistProxy.getLabList()[curLabIndex.value].getAsignedCourseList().getAllAsignedCourse();
    let ok=true;
    ...
    for(let i=0;i<clab.length;i++)
    {
        ...
        if(cweek<=c.EndWeek&&cweek>=c.StartWeek&&dayIndex==c.TeachingDay&&timeIndex<=c.EndTime&&timeIndex>=c.StartTime)
        {
            ok=false;
        }
    }
  ...
  let handleConfirmation = (): void => {
    ...
    let count= ct.totalTime -(t.EndWeek-t.StartWeek+1)*(t.EndTime-t.StartTime+1)-ct.curTimeSum;
    console.log("cost: "+(t.EndWeek-t.StartWeek+1)*(t.EndTime-t.StartTime+1)+"curtimSum:"+ct.curTimeSum);
    ...
    for(let i=0;i<ObjLab.length;i++)
    {
       ...
       for(let j=ObjLab[i].StartWeek;j<=ObjLab[i].EndWeek;j++)
       {
         for(let k=ObjLab[i].StartTime;k<=ObjLab[i].EndTime;k++)
         {
            ...
            if(j>=t.StartWeek&&j<=t.EndWeek&&k>=t.StartTime&&k<=t.EndTime&&ObjLab[i].TeachingDay==t.teachingDay)
            {
    ...
    let tCourse=c.getAllCourses()[curCourseIndex.value];
    console.info("tCourseName:"+curCourseIndex.value);
    if(t.StartWeek<=t.EndWeek&&t.StartTime<=t.EndTime&&t.StartTime<=18&&t.StartTime>=1&&t.EndTime<=18&&t.StartTime>=1&&t.EndTime<=8&&selectedCourseId.value>=1&&Noconflict&&count>=0&&t.teachingDay>=1&&t.teachingDay<=7)
    {
...
let confirmBookingModal=()=>{
    let t = tAsignedCourseParameter.value;
    let clablistCourses=lablistProxy.getLabList()[curLabIndex.value].asignedCourseList.getAllAsignedCourse();
    let cindex=selectedCourseId.value-1
    
    clablistCourses[cindex].updateCourseTime((t.EndWeek-t.StartWeek+1)*(t.EndTime-t.StartTime+1));
    
    //cweek,cLab
    let ct= clablistCourses[cindex];
    let newAsignedCourse=new AsignedCourse(ct,t.StartWeek,t.EndWeek,t.teachingDay,t.StartTime,t.EndTime);
  ...
  watch(curLabIndex, (newValue, oldValue) => {
    ...
    // 重置或更新其他状态
    tAsignedCourseParameter.value = { StartWeek: 0, EndWeek: 0, teachingDay: 0, StartTime: 0, EndTime: 0 };
...
watch(curLabIndex, (newValue) => {
  ...
  ObjLab = reactive(lablist.getLabList()[newIndex < 0 ? 0 : newIndex].getAsignedCourseList().getAllAsignedCourse());
});
...
<template>
    <div>
      <div class="head">
        ...
        <div style="display: inline-flex;">
          ...
          <button style="display: inline-flex;background-color: skyblue;padding: 2px;border-radius: 5px;color: white;" @click="openModal">
            添加机房
          </button>
        </div>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div class="modal-content">
        ...
        <option v-for="course in courselist.getAllCourses()" :key="course.courseId" :value="course.courseId">
            {{ course.courseName }}
        </option>
        </select>
        <div>
            <label  for="startWeek">[1-18]起始周:</label>
            <input @change="handleConfirmation" type="text" id="startWeek" name="startWeek" v-model="tAsignedCourseParameter.StartWeek">
        </div>
        <div>
            <label  for="endWeek">[1-18]结束周:</label>
            <input @change="handleConfirmation"  type="text" id="endWeek" name="endWeek" v-model="tAsignedCourseParameter.EndWeek">
        </div>
        <div>
            <label  for="endClass">[1-7]星期几:</label>
            <input @change="handleConfirmation" type="text" id="endClass" name="endClass" v-model="tAsignedCourseParameter.teachingDay">
        </div>
        <div>
            <label for="startClass">[1-8]起始节课:</label>
            <input  @change="handleConfirmation" type="text" id="startClass" name="startClass" v-model="tAsignedCourseParameter.StartTime">
        </div>
        <div>
            <label  for="endClass">[1-8]终止节课:</label>
            <input @change="handleConfirmation" type="text" id="endClass" name="endClass" v-model="tAsignedCourseParameter.EndTime">
        </div>
...
<!-- //已分配课表 -->
    ...
    <tr v-for="(c, index) of refLab" :key="index" :style="{backgroundColor: c.curTimeSum >= c.totalTime ? 'yellow' : 'lightgreen'}">
        <td style="text-align: center;">{{ index + 1}}</td>
        <td style="text-align: center;">{{ c.courseName}}</td>
        <td style="text-align: center;">{{ c.totalTime }}</td>
        <td style="text-align: center;"> &nbsp;&nbsp;已预约课时{{}}{{ c.StartWeek }}-{{ c.EndWeek }}周，星期{{ c.TeachingDay }},{{ c.StartTime }}-{{ c.EndTime }}节，{{ c.teacher.teacherName }},{{ c.teacher.TeacherTitle}}&nbsp;&nbsp;</td>
...
<div style="display: inline-flex; color: aqua;">
    <h3>空闲课表</h3>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
          ...
          <select @change="getWeekofWeekSelect"  name="Week" id="Week">
            <option v-for="(c,index) in Week" :key="index" :value="c">
            第{{c}}周
            </option>
        </select>
      ...
      <tbody>
        ...
        <td>{{ time }}</td>
        <td v-for="(day, dayIndex) in days" :key="dayIndex" :style="{ backgroundColor: getBackgroundColor(dayIndex+1, timeIndex+1) }">
